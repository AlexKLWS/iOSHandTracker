...
void HandGesture::initVectors(){
	hullIndex=vector<vector<int> >(contours.size());
	hullPoint=vector<vector<cv::Point> >(contours.size());
...
void HandGesture::printGestureInfo(Mat src){
	...
	xpos+=lineChange;
	info=string("Number of defects: ") + string(intToString(numberOfDefects)) ;
	putText(src,info,cv::Point(ypos,xpos),fontFace,fontSize  ,fColor);
	xpos+=lineChange;
...
void HandGesture::addNumberToImg(ImageSource *m){
	int xPos=10;
	int yPos=10;
	int offset=30;
	float fontSize=1.5f;
	...
	for(int i=0;i<numbers2Display.size();i++){
		rectangle(m->original,cv::Point(xPos,yPos),cv::Point(xPos+offset,yPos+offset),numberColor, 2);	
		putText(m->original, intToString(numbers2Display[i]),cv::Point(xPos+7,yPos+offset-3),fontFace,fontSize,numberColor);
		xPos+=40;
		if(xPos>(m->original.cols-m->original.cols/3.2)){
			yPos+=40;
			xPos=10;
		}
	}
...
// over 20 frames
void HandGesture::getFingerNumber(ImageSource *m){
	removeRedundantFingerTips();
	if(bRect.height > m->original.rows/2 && nrNoFinger>12 && isHand ){
...
void HandGesture::drawFingertips(ImageSource *m){
	...
	for(int i=0;i<fingerTips.size();i++){
		...
		putText(m->original,intToString(i),p-cv::Point(0,30),fontFace, 1.2f,Scalar(200,200,200),2);
   		circle( m->original,p,   5, Scalar(100,255,100), 4 );
   	 }
}

void HandGesture::getFingertips(ImageSource *m){
	fingerTips.clear();
	int i=0;
	vector<Vec4i>::iterator d=defects[cIdx].begin();
	while( d!=defects[cIdx].end() ) {
